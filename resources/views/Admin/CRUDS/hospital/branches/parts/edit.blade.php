...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('hospitals.update',$row->id)}}">
    ...
    <div class="row g-4">
        ...
        <div class="d-flex flex-column mb-12 fv-row col-sm-12">
            <!--begin::Label-->
            ...
            <select name="type" id="type" class="form-control">
                <option value="hospital" {{$row->type == 'hospital' ? 'selected' : ''}}>Hospital</option>
                <option value="center" {{$row->type == 'center' ? 'selected' : ''}}>Center</option>
            </select>
        </div>

        <div class="d-flex flex-column mb-12 fv-row col-sm-12">
            <!--begin::Label-->
            <label for="hospital_id" class="d-flex align-items-center fs-6 fw-bold form-label mb-2">
                <span class="required mr-1">{{helperTrans('admin.Hospitals')}} <span class="red-star">*</span></span>
            </label>

            <select name="hospital_id" id="hospital_id" class="form-control">
                @foreach ($hospitals as $item)
                    <option value="{{$item->id}} {{$row->hospital_id == $item->id ? 'selected' : ''}}">{{$item->name}}</option>
                @endforeach
            </select>
        </div>
